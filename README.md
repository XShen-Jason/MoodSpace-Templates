...
## 📄 核心枢纽：`config.json` 字段深度剖析

当你新增一号模板时，请严格编写这个控制端 JSON 文件。

### 基础元数据
- `"name"`: (String) 技术标识名，务必一字不差地和此模板的**文件夹名称**保持一致。
- `"title"`: (String) 界面显示名称（支持中文）。用户在网站“随便逛逛”时看到的就是这个美丽的名字。
- `"tier"`: (String) **收费隔离分级**。填 `"free"` 则所有人都能免费用；填 `"pro"` 则只有尊贵的高级会员能使用。不写默认当 free 处理。
- `"price"`: (Number) **单价**。如果你的系统设计了普通人能“单买拔草”的功能，可标明售价比如 `9.9`。
- `"static"`: (Boolean) 填写 `true` 时，意为这不是个能定制的网页。用户点进来只能硬看源码展示。

### `"fields"` 动态配置项（也就是向用户暴露的输入框）
这是一个 JSON 数组（Array）。你在这里定义几个，前端编辑器的左侧就会自动生出几个输入框！

| 字段名 | 含义 | 对应前台展现形态 |
| :--- | :--- | :--- |
| `id` | 数据键名（至关重要） | 如果在 JSON 写了 `"id": "msg"`, 那么你的 `index.html` 源码里出现的所有 `{{msg}}` 占位符都会在这个网页被生成时遭到替换。 |
| `label` | 人类能懂的中文标题标签 | 比如叫 `浪漫告白文案`。显示在左侧输入框正上方。 |
| `type` | 格式类型 | 支持 `text` (单行框) 或 `textarea` (高高的多行框)。 |
| `default` | 默认占位美化值 | 用户如果懒得输入字，这个默认值就是拯救整个原本光秃秃页面的底线。 |
| `placeholder`| 提示暗语 | 用灰色字体躺在输入框里的诱导文本，例如“告诉 TA 你的真心话…” |
